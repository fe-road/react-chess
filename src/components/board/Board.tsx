...
const Board = ({ board, playerTurn, playingAsWhite, movePiece }: Props) => {
    ...
    const isSameAsSelectedSquare = (square: SquareModel): boolean => {
        return square?.coordinates.row === selectedSquare?.coordinates.row && square?.coordinates.column === selectedSquare?.coordinates.column;
    }
    ...
    return (
        <section className='grid grid-rows-8 grid-cols-8 max-w-xl w-3/4 aspect-square my-4 mx-auto border shadow'>
